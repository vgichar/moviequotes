...
module DB{
	...
	export class SeriesDB extends BaseDB<SerieModel>{
		public constructor(){
			super("json/series.json");
		}

		public all(): linq.Enumerable<SerieModel> {
			let all = super.all().Select(x => new SerieModel(x.id, x.title, x.year, x.img));
			return all;
		}

		public getByStart(str: string): linq.Enumerable<SerieModel> {
			if(str && str.length > 0){
				str = str.toLowerCase();
		...
		public getByContains(str: string): linq.Enumerable<SerieModel> {
			if(str && str.length > 0){
				str = str.toLowerCase();
				return this.all().Where(x => x.title.toLowerCase().indexOf(str) >= 0);
			}else{
		...
		public take (offset, take): linq.Enumerable<SerieModel> {
			return this.all().Skip(offset).Take(take);
		}

		public get (slug): SerieModel {
			return this.all().Single(x => x.slug == slug);
		}

		public getMovieOfTheDay (): SerieModel {
			let content: any = this.getPopularContent();
			let movie = content['serie-of-the-day'];
			return new SerieModel(movie.id, movie.title, movie.year, movie.img);
		}

		public getPopularMovies (): linq.Enumerable<SerieModel> {
			let content: any = this.getPopularContent();
			let movies: any[] = content['popular-series'];

			return Enumerable.From(movies).Select(x => new SerieModel(x.id, x.title, x.year, x.img));
		}

		public getRandomMovie (notIn: string[] = []): SerieModel {
